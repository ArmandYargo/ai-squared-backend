...
# -------------------------------------------------------------------
# The RAM input sheets may contain blanks; when read by pandas these become NaN.
...
def _safe_int(x, default=0, min_v=None, max_v=None):
    ...
    try:
        if x is None:
            ...
            v = default
        elif isinstance(x, (np.floating,)) and np.isnan(x):
            v = default
        else:
            v = int(float(x))
    except Exception:
...
def find_maintenance_practice(date, n):
    # Return (maintenance_practice, rating) for timeline n at `date`.
    # If date is beyond the last `to`, fall back to last row.
    tl = globals()[f'timeline_{n}'].copy()
    tl['to'] = pd.to_datetime(tl['to'], errors='coerce')
    if len(tl) == 0:
        raise ValueError(f'timeline_{n} is empty')
    date_ts = pd.Timestamp(date)
    idx = tl.index[tl['to'] >= date_ts]
    i = int(idx[0]) if len(idx) else int(tl.index[-1])
    mp = str(tl.loc[i, 'maintenance_practice'])
    rating_raw = tl.loc[i, 'rating']
    rating = _safe_int(rating_raw, default=1, min_v=1, max_v=4)
    return mp, rating


def find_serv(date,n):
    # Return (serv_ind, pro_active_ind) for timeline n at `date`.
    # NaN-safe + falls back to last row when date beyond last `to`.
    tl = globals()[f'timeline_{n}'].copy()
    tl['to'] = pd.to_datetime(tl['to'], errors='coerce')
    if len(tl) == 0:
        raise ValueError(f'timeline_{n} is empty')
    ...
    idx = tl.index[tl['to'] >= date_ts]
    i = int(idx[0]) if len(idx) else int(tl.index[-1])
    serv_ind = _safe_int(tl.loc[i, 'serv_ind'], default=0, min_v=0, max_v=1)
    pa_ind = _safe_int(tl.loc[i, 'pro_active_ind'], default=0, min_v=0, max_v=1)
    return serv_ind, pa_ind

def find_next_maintenance_practice(date, time_step, time_to_event_2, time_to_change, n):
    # Determine whether the maintenance practice changes before next event.
    # Robust handling when `date` is beyond the last `to` date.
    timeline_change = globals()[f'timeline_{n}'].copy()
    timeline_change['from'] = pd.to_datetime(timeline_change['from'], errors='coerce')
    timeline_change['to'] = pd.to_datetime(timeline_change['to'], errors='coerce')
    if len(timeline_change) == 0:
        return 'no', 0, 0, None, None, None, None
    ...
    date_ts = pd.Timestamp(date)
    idx = timeline_change.index[timeline_change['to'] >= date_ts]
    i = int(idx[0]) if len(idx) else int(timeline_change.index[-1])

    # If we are already in the last row, there cannot be a future change
    if i >= (len(timeline_change) - 1):
        return 'no', 0, 0, None, None, None, None

    # Does the next event happen after the current timeline row ends?
    if (time_step + time_to_event_2) > timeline_change.loc[i, 'acc_time'].days:
        mp_change = 'yes'
        time_to_change_2 = timeline_change.loc[i, 'acc_time'].days - time_step
        time_to_change = time_to_change + time_to_change_2
        next_mp = str(timeline_change.loc[i + 1, 'maintenance_practice'])
        next_rating = _safe_int(timeline_change.loc[i + 1, 'rating'], default=1, min_v=1, max_v=4)
        next_serv_ind = _safe_int(timeline_change.loc[i + 1, 'serv_ind'], default=0, min_v=0, max_v=1)
        next_pa_ind = _safe_int(timeline_change.loc[i + 1, 'pro_active_ind'], default=0, min_v=0, max_v=1)
...
def maintenance_strings_to_code(maintenance_practice,rating):
    ...
    rating_i = _safe_int(rating, default=1, min_v=1, max_v=4)
    return maintenance_practice_to_code[maintenance_practice] + rating_i
