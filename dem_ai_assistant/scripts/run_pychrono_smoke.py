"""
Quick check that PyChrono is installed and the transfer-slice solver runs.
From project root: python scripts/run_pychrono_smoke.py
"""
...
def main() -> None:
    ...
    g = load_yaml_config(PROJECT_ROOT / "configs" / "geometry" / "transfer_point_template.yaml")
    ...
    material, scenario, geometry = m, s, g

    sim_input = build_simulation_input(
        material, scenario, geometry, project_root=PROJECT_ROOT
    )

    out = run_pychrono_transfer_slice(sim_input)
    print(out.notes)
    print("egress proxy:", out.flow_stats.estimated_spill_events)
    print("min edge margin mm:", out.edge_stats.min_edge_margin_mm)
