...
def _normalize_mp(mp: Any) -> str:
    """Normalize maintenance practice strings from Excel/user inputs.

    Handles common case/spacing variants and known typos like 'Care&Maintance'.
    """
    if mp is None:
        return ""
    s = str(mp).strip().lower()
    s = " ".join(s.split())
    mapping = {
        "reactive": "Reactive",
    ...
    return mapping.get(s, str(mp).strip())
## Model logic
...
def time_to_event_func(ctx, comp, mp, rating, serv_ind, pa_ind, time_step_opr, prev_event_type=None, prev_damage=0, prev_weib_tf=None, acc_time_to_event=None, time_to_change=0, mp_change = "no"):
    ...
    # --- Normalize inputs (prevents crash on typos/NaNs) ---
    mp = _normalize_mp(mp)
    try:
        # rating can be NaN/float from Excel
        rating = int(rating)
    except Exception:
        rating = 1
...
######################reactive & preventative maintenance practices########################################################################
        ...
        event_type = 1
        mp_assigned = "Reactive"
        time_to_event_2 = max(1, round(pf_curve_t(ctx, cond=0, d_i=prev_damage, n=ctx.comp_att_df.loc[comp].loc["pf_n"], tf=weib_tf), 0))
        time_to_event = time_to_change + time_to_event_2
        cond_at_event = max(0, round(pf_curve_cond(ctx, t=time_to_event_2, d_i=prev_damage, n=ctx.comp_att_df.loc[comp].loc["pf_n"], tf=weib_tf), 2))
        prev_event_type = 1
        prev_weib_tf = weib_tf
