...
def find_maintenance_practice(date, n):
    ...
    PATCH: if `date` is beyond the last 'to' in the timeline, fall back to the
    last row instead of returning None (which later causes unpack errors).
    """
    ...
    # Find first row whose 'to' is >= date; else use last row
    idx = tl.index[tl['to'] >= date_ts]
    i = int(idx[0]) if len(idx) else int(tl.index[-1])
    return str(tl.loc[i, 'maintenance_practice']), float(tl.loc[i, 'rating'])


def find_serv(date, n):
    """Return (serv_ind, pro_active_ind) for timeline n at `date`.

    PATCH: same safe fallback behaviour as find_maintenance_practice.
    """
    tl = globals()[f'timeline_{n}'].copy()
    tl['to'] = pd.to_datetime(tl['to'], errors='coerce')
    if len(tl) == 0:
    ...
    idx = tl.index[tl['to'] >= date_ts]
    i = int(idx[0]) if len(idx) else int(tl.index[-1])
    return int(tl.loc[i, 'serv_ind']), int(tl.loc[i, 'pro_active_ind'])
...
def find_next_maintenance_practice(date, time_step, time_to_event_2, time_to_change, n):
    ...
    PATCH: robust handling when `date` is beyond the last 'to' date.
    """
    ...
    if len(timeline_change) == 0:
        # no timeline entries -> no change
        return 'no', 0, 0, None, None, None, None

    # Build accumulated time
    ...
    # Default: no change
    if (time_step + time_to_event_2) > timeline_change.loc[i, 'acc_time'].days and i < (len(timeline_change) - 1):
        mp_change = 'yes'
        ...
        next_rating = float(timeline_change.loc[i + 1, 'rating'])
        next_serv_ind = int(timeline_change.loc[i + 1, 'serv_ind'])
        next_pa_ind = int(timeline_change.loc[i + 1, 'pro_active_ind'])
        return mp_change, time_to_change_2, time_to_change, next_mp, next_rating, next_serv_ind, next_pa_ind

    mp_change = 'no'
    return mp_change, 0, 0, None, None, None, None
...
# --- agent-safety: normalize maintenance practice labels from Excel / user input ---
def _normalize_mp(mp):
    ...
    mapping = {
        ...
        'care&maintnance': 'Care&Maintenance',
    }
    return mapping.get(s, ('' if mp is None else str(mp)).strip())
...
######################reactive & preventative maintenance practices########################################################################
        ...
        # Fallback: unknown/unsupported maintenance practice.
        # Keep the simulation running by treating it as Reactive.
        event_type = 1
        mp_assigned = 'Reactive'
        time_to_event_2 = max(1, round(pf_curve_t(cond=0, d_i=prev_damage, n=comp_att_df.loc[comp].loc['pf_n'], tf=weib_tf), 0))
        time_to_event = time_to_change + time_to_event_2
        cond_at_event = max(0, round(pf_curve_cond(t=time_to_event_2, d_i=prev_damage, n=comp_att_df.loc[comp].loc['pf_n'], tf=weib_tf), 2))
        prev_event_type = 1
        prev_weib_tf = weib_tf
